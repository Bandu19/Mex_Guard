...
export const contPendientesHtml = (todo) => {
  // IMPRIMIENDO HTML
  const contHtmlTodo = `<strong>${todo.id}</strong> pendiente(s)`;

  const span = document.createElement("span");
  span.innerHTML = contHtmlTodo;
...
ulFiltros.addEventListener("click", (event) => {
  // todoList.contarPendientes();
  // console.log("R2:", todoList.todos.length);
  ...
  console.log("TOTAL:", divTodoList.children.completado);
  ...
  for (const elemento of divTodoList.children) {
    ...
    const completado = elemento.classList.contains("completed"); //VERIFICAR SI TIENEN
